...
def write_sheet(workbook: xlsxwriter.Workbook, name: str, rows: list[StockRow]) -> None:
    ...
    ws.write(0, 0, f"Реальные остатки Wildberries — {name}", title_fmt)
    ws.write(1, 0, f"Сформировано: {datetime.now():%d.%m.%Y %H:%M}")
    ws.write(2, 0, "Реальный FBO берётся из покупательской витрины WB (dtype=4). FBS — из API склада продавца.")
    ...
    for i, r in enumerate(rows, start=header_row + 1):
        ...
        for col, value in enumerate(values):
            fmt = cell if col in (0, 1, 3) else center
            if col == 9:
    ...
    # Дополнительная визуализация скрытого остатка.
    if rows:
        ws.conditional_format(
            header_row + 1, 7, header_row + len(rows), 7,
            {"type": "data_bar", "bar_color": "#F4B183"}
        )
...
def export_workbook(
    ...
    workbook.set_properties({
        "title": "Реальные остатки Wildberries",
        "subject": "FBO / FBS / скрытые остатки",
        "author": "WB Real Stock Bot",
    })
